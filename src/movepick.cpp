...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        if constexpr (Type == CAPTURES)
        {
            ...
            Square    to   = m.to_sq();
            PieceType capt = type_of(pos.piece_on(to));
            m.value        = 7 * int(PieceValue[capt]) + (*captureHistory)[pc][to][capt]
                    + (*continuationHistory[0])[pc][to] / 8
                    + (*continuationHistory[1])[pc][to] / 16;
        }
